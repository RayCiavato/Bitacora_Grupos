...
test("bundles minificados no comparten helpers globales de selectores DOM", () => {
  const appBundlePath = path.join(__dirname, "..", "src", "public", "assets", "app.min.js");
  const tasksBundlePath = path.join(__dirname, "..", "src", "public", "assets", "tasks.min.js");
  const appBundle = fs.readFileSync(appBundlePath, "utf8");
  const tasksBundle = fs.readFileSync(tasksBundlePath, "utf8");

  assert.match(appBundle, /const _0x5a_app=/);
  assert.match(tasksBundle, /const _0x5a_tasks=/);
  assert.doesNotMatch(appBundle, /const _0x5a=/);
  assert.doesNotMatch(tasksBundle, /const _0x5a=/);
});
