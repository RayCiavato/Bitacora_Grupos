...
CREATE TABLE IF NOT EXISTS tasks (
  ...
  status VARCHAR(32) NOT NULL DEFAULT 'sin_realizar' CHECK (status IN ('sin_realizar', 'en_proceso', 'pendiente_revision', 'completada', 'cancelada')),
  ...
  created_at TIMESTAMPTZ NOT NULL DEFAULT NOW(),
  updated_at TIMESTAMPTZ NOT NULL DEFAULT NOW(),
  CONSTRAINT tasks_due_after_start_check CHECK (due_date IS NULL OR start_date IS NULL OR due_date >= start_date)
);
